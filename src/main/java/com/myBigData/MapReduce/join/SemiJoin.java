...
public class SemiJoin extends Configured implements Tool{
    ...
    public static class SemiJoinMapper extends Mapper<Object, Text, Text, CombineValues> {
        ...
        /**
         * 将参加join的key从DistributedCache取出放到内存中，以便在map端将要参加join的key过滤出来。
         */
        @Override
        protected void setup(Context context)  throws IOException, InterruptedException {
            Text joinKeyStr=new Text();
            try {
                ...
                for(URI uri : uris){
                    ...
                    if (path.toString().endsWith("tb_dim_city.dat")) {
                        ...
                        while(lineReader.readLine(joinKeyStr)>0){
                            if (joinKeyStr!=null ) {
                                String[] splits = joinKeyStr.toString().split("\\|");
    ...
    public int run(String[] args) throws Exception {
        ...
        FileInputFormat.addInputPath(job, new Path(args[1])); //设置map输入文件路径
